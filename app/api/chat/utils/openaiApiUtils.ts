// src/app/api/chat/utils/openaiApiUtils.ts

// --- IMPORTS ---
// Import constants needed
import {
  ConversationEntry
} from '../types/routeTypes'; // Corrected path: Go up one level, then into types

// Import validation instructions if needed (only default is used below)
import {
  defaultValidationInstruction
} from '../prompts/validationInstructions'; // Corrected path: Go up one level, then into prompts

// --- TYPES (Define if not imported) ---
// Define the type for conversation history entries if not globally available/imported
// type ConversationMessage = { role: string; content: string }; // Can use ConversationEntry now
...
history: ConversationEntry[] // Changed type to ConversationEntry
): Promise<string> {
...
const payload = {
  ...
  messages: [
    ...
    ...history.filter((entry: ConversationEntry) => entry.role !== "system"),
...
try {
  ...
  if (!response.ok) {
    ...
    if (errorData.error?.code === "rate_limit_exceeded") {
      return `Rate limit reached: ${errorData.error.message ?? 'unknown reason'}`;
    }
