...
async function validateInput(userInput: string, currentPrompt: string) {
  const validationInstruction = `
    ...
    Your task is to assess if the user's has responded to the prompt requirements.  Don't be too strict with this.  
    As long as the answer makes sense to the prompt it is VALID.  If the answer is completely unrelated it isn't valid.
    Current prompt: '${currentPrompt}'
    User input: '${userInput}'
    
    Respond with only one word: "VALID" if the input is appropriate to the prompt,
