...
if (PROMPT_LIST[currentIndex - 1]?.important_memory) {
  ...
  console.log("[DEBUG] Important Memory Added to Conversation History at index:", insertIndex);
      ...
      conversationHistory = manageBuffer(conversationHistory);
      console.log("[DEBUG] Separate AutoTransition Responses:");
      console.log("[DEBUG] First Response:\n", assistantContent1);
      console.log("[DEBUG] Second Response:\n", assistantContent2);
      
      // Only return the second response to the client
      return new Response(assistantContent2, { status: 200, headers: { "Content-Type": "text/plain" } });
