...
const defaultValidationInstruction = `
  ...
  Your task is to assess if the user's inputs the name of the book 'moby dick'. 

 if the user's inputs the name of moby dickk, it is VALID. If user doesn't input moby dick, it is INVALID.
  Current prompt: '{CURRENT_PROMPT}'
  User input: '{USER_INPUT}'

  Respond with only one word: "VALID" if the user inputs moby dick ,
  or "INVALID" if not.
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (PROMPT_LIST[currentIndex]?.validation) {
      ...
      // Get the validation property from the prompt.
      // If it is a string, we pass it as a custom instruction; if it's simply true, we pass nothing.
