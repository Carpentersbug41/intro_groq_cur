...
const PROMPT_LIST = [
  { prompt_text: "Ask the user what their favorite color is.", important_memory: true   },
  { prompt_text: "Ask the user what their favorite dish is.",autoTransition: true, },
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!assistantContent1) {
      console.warn("[WARN] First API call returned no content. Skipping further calls.");
    ...
    // Handle multiple auto transitions
    if (isAutoTransition) {
      const autoResponses: string[] = [];
      autoResponses.push(assistantContent1);

      // Process consecutive auto-transition prompts
      while (true) {
        // Simulate user input "OK" to advance
        ...
        const nextAutoPrompt = PROMPT_LIST[currentIndex];
        const nextAutoPromptText = nextAutoPrompt?.prompt_text || "No further prompts.";
        conversationHistory = [
          { role: "system", content: nextAutoPromptText },
          ...conversationHistory.filter((entry) => entry.role !== "system"),
        ];
        console.log("[DEBUG] Updated Conversation History for Next Auto-Transition:\n", JSON.stringify(conversationHistory, null, 2));

        // Apply buffer management before the next payload
        conversationHistory = manageBuffer(conversationHistory);
        ...
        const assistantContentN = await fetchApiResponseWithRetry(payloadN, 2, 500);
        if (!assistantContentN) {
          console.warn("[WARN] API call failed during auto-transition. Returning partial results.");
          const partialCombined = autoResponses.join("\n\n");
          return new Response(partialCombined, { status: 200 });
        }

        conversationHistory.push({ role: "assistant", content: assistantContentN });
        console.log("[DEBUG] Another AutoTransition Response Added to Conversation History.");
        autoResponses.push(assistantContentN);

        // If the current prompt is marked as important_memory, add it
        if (PROMPT_LIST[currentIndex - 1]?.important_memory) {
          const systemIndex = conversationHistory.findIndex((msg) => msg.role === "system");
          ...
          conversationHistory.splice(insertIndex, 0, {
            role: "assistant",
            content: `Important_memory: ${assistantContentN}`,
          });
          console.log("[DEBUG] Important Memory Inserted after Another AutoTransition.");
        }

        // If the next prompt is not autoTransition, break the loop
        if (currentIndex + 1 >= PROMPT_LIST.length || !PROMPT_LIST[currentIndex + 1]?.autoTransition) {
          console.log("[DEBUG] Next prompt is NOT autoTransition. Breaking loop.");
          break;
        }
      }

      // Combine responses with line breaks
      const combinedResponses = autoResponses.join("\n\n");
      console.log("[DEBUG] Final Combined Auto-Transition Responses:\n", combinedResponses);
      return new Response(combinedResponses, { status: 200 });
    }
